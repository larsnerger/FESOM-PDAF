...
!   use g_comm_auto, &
!        only: gather_nod, exchange_nod, exchange_elem, broadcast_nod
!   use g_config, &
...
!        zbar_n_bot, zbar_n_srf, hnode_new, z_n, zbar_n
  use o_param, &
       only: r_earth, rad, pi !, num_tracers
!   use i_arrays, &
!        only: a_ice
  use recom_config, &
...
contains
  ...
  subroutine set_fesom_pdaf_vars()
    ...
    MPI_COMM_FESOM = f%partit%MPI_COMM_FESOM

    mydim_nod2d = f%partit%mydim_nod2d
    edim_nod2d = f%partit%edim_nod2d
    mydim_elem2d = f%partit%mydim_elem2d
    edim_elem2d = f%partit%edim_elem2d
