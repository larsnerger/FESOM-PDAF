...
MODULE obs_Alk_glodap_pdafomi
  ...
  LOGICAL :: assim_o_Alk_glodap   !< Whether to assimilate

  ! Further variables specific for profile observations
  CHARACTER(len=110) :: path_obs_Alk_glodap  = ''      !< Path to profile observations
  CHARACTER(len=110) :: file_Alk_glodap

  REAL    :: rms_obs_Alk_glodap      ! Observation error
  REAL    :: bias_obs_Alk_glodap     ! Observation bias

  REAL    :: lradius_Alk_glodap      ! Localization radius
  REAL    :: sradius_Alk_glodap      ! Support radius for localization function

  REAL    :: Alk_glodap_exclude_diff ! Limit difference beyond which observations are excluded (0.0 to deactivate)
...
!!
  SUBROUTINE init_dim_obs_Alk_glodap(step, dim_obs)
...
! *********************************************
    ...
    lradius_Alk_glodap = 1.0e6 ! 1000 km
    sradius_Alk_glodap = 1.0e6 ! 1000 km
    ...
    thisobs%inno_omit =  Alk_glodap_exclude_diff / rms_obs_Alk_glodap
...
! **********************************
    ...
    file_Alk_glodap = TRIM(year_string//'-'//mon_string//'-'//day_string//'.'//mype_string//'.nc')
    ...
    IF (step < 0) then
        ...
        if (writepe) WRITE (*,'(a,5x,a,1x,i2,a1,i2,a1,i4,1x,f5.2,a,1x,a)') &
            'FESOM-PDAF', 'Postprocessing of GLODAP ALK observations at', &
            day_in_month, '.', month, '.', yearnew, timenew/3600.0,&
            'h; read from file:', file_Alk_glodap
    ...
    ELSE
       ...
       ! ---
       if (writepe) WRITE (*,'(a,5x,a,1x,i2,a1,i2,a1,i4,1x,f5.2,a,1x,a)') &
            'FESOM-PDAF', 'Assimilate GLODAP ALK observations at', &
            day_in_month, '.', month, '.', yearnew, timenew/3600.0,&
            'h; read from file:', file_Alk_glodap
       ...
       IF (assim_o_Alk_glodap) thisobs%doassim = 1
    ENDIF

    ! complete path + filename
    filename=TRIM(path_obs_Alk_glodap)//year_string//'/dist72/'//file_Alk_glodap
    INQUIRE(file=TRIM(filename), exist=FileExists)
    IF (FileExists) THEN
       ! Exists: Open the pe-local NetCDF file for that day
       ncstat = nf90_open(TRIM(filename), nf90_nowrite, ncid)
       if (ncstat /= nf90_noerr) then
        print *, 'FESOM-PDAF - obs_Alk_glodap_pdafomi - Error opening NetCDF file'
       end if
       ...
       if (ncstat /= nf90_noerr) then
          print *, "FESOM-PDAF - obs_Alk_glodap_pdafomi - Error getting dimension ID"
       end if
       ncstat = nf90_inquire_dimension(ncid,dimid,len=dim_obs_p)
       if (ncstat /= nf90_noerr) then
          print *, 'FESOM-PDAF - obs_Alk_glodap_pdafomi - Error getting number of observations from NetCDF file'
       end if
    ELSE
       ! Not existing:
       dim_obs_p=0
    ...
    ELSE ! (i.e. dim_obs_p > 0)
      ...
      print *, 'FESOM-PDAF - obs_Alk_glodap_pdafomi - Have obs: ', dim_obs_p
      
      ! Reading observations
      ncstat = nf90_inq_varid(ncid,'OBS', id_obs)
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_Alk_glodap_pdafomi - Error getting id_obs from netCDF'
      ncstat = nf90_get_var(ncid, id_obs, obs_p)
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_Alk_glodap_pdafomi - Error reading obs from netCDF'
   
      ! Reading nodes
      ncstat = nf90_inq_varid(ncid,'NOD1_P', id_nod1)
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_Alk_glodap_pdafomi - Error getting id_nod1 from netCDF'
      ncstat = nf90_get_var(ncid, id_nod1, nod1_p)
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_Alk_glodap_pdafomi - Error reading nod1 from netCDF'
   
      ncstat = nf90_inq_varid(ncid,'NOD2_P', id_nod2)
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_Alk_glodap_pdafomi - Error getting id_nod2 from netCDF'
      ncstat = nf90_get_var(ncid, id_nod2, nod2_p)
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_Alk_glodap_pdafomi - Error reading nod2 from netCDF'
   
      ncstat = nf90_inq_varid(ncid,'NOD3_P', id_nod3)
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_Alk_glodap_pdafomi - Error getting id_nod3 from netCDF'
      ncstat = nf90_get_var(ncid, id_nod3, nod3_p)
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_Alk_glodap_pdafomi - Error reading nod3 from netCDF'
   
      ! Reading layer
      ncstat = nf90_inq_varid(ncid,'NZ1', id_nl)
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_Alk_glodap_pdafomi - Error getting id_layer from netCDF'
      ncstat = nf90_get_var(ncid, id_nl, nl_p)
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_Alk_glodap_pdafomi - Error reading layer from netCDF'
   
      ! Reading coordinates
      ncstat = nf90_inq_varid(ncid,'LON', id_lon)
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_Alk_glodap_pdafomi - Error getting id_lon from netCDF'
      ncstat = nf90_get_var(ncid, id_lon, lon_p)
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_Alk_glodap_pdafomi - Error reading lon from netCDF'
   
      ncstat = nf90_inq_varid(ncid,'LAT', id_lat)
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_Alk_glodap_pdafomi - Error getting id_lat from netCDF'
      ncstat = nf90_get_var(ncid, id_lat, lat_p)
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_Alk_glodap_pdafomi - Error reading lat from netCDF'
      ...
      IF (mype_filter == 18) &
         ...
         '--- Use global GLODAP Alk observation error of ', rms_obs_Alk_glodap, ' mmol/m3'
      ! Set inverse observation error variance
      ivariance_obs_p = 1.0 / (rms_obs_Alk_glodap ** 2)
    ...
    IF (mype_filter == 0) &
            ...
            '--- Alk GLODAP observations excluded due to topography: ', cnt_ex_dry
    ...
    CALL PDAFomi_gather_obs(thisobs, dim_obs_p, obs_p, ivariance_obs_p, ocoord_p, &
                            thisobs%ncoord, lradius_Alk_glodap, dim_obs)
    ...
    if (FileExists) then
       ...
       if (ncstat /= nf90_noerr) then
          print *, 'FESOM-PDAF - obs_Alk_glodap_pdafomi - Error closing NetCDF file'
       end if
    end if
  ...
  END SUBROUTINE init_dim_obs_Alk_glodap
...
!!
  SUBROUTINE obs_op_Alk_glodap(dim_p, dim_obs, state_p, ostate)
...
! ******************************************************
  ...
  END SUBROUTINE obs_op_Alk_glodap
...
!!
  SUBROUTINE init_dim_obs_l_Alk_glodap(domain_p, step, dim_obs, dim_obs_l)
    ...
    IF (thisobs%doassim == 1) THEN
       ...
       if (n_sweeps>1) then
          ...
          elseif (domain_p==myDim_nod2D+1) then
             if (mype_filter==0) &
                  write (*,'(a,4x,a)') 'FESOM-PDAF', &
                  '--- BIO sweep: set ivar_obs_f for ALK to original'
...
END MODULE obs_Alk_glodap_pdafomi
