!$Id: obs_sst_cmems_pdafomi.F90 2543 2021-05-13 08:17:31Z lnerger $
...
module obs_sst_pdafomi
  ...
  ! Further variables specific for the OSTIA SST observations
...
! **********************************
    ...
    obs_file=trim(file_sst_prefix)//trim(mype_string)//trim(file_sst_suffix)
    ...
    stat(1) = NF_OPEN(trim(path_obs_sst)//trim(obs_file), NF_NOWRITE, fileid)        
 
    ! *** Read obs state estimate ***
...
! ***********************************************************
    ...
    do i = 1, myDim_nod2D
       if (abs(all_obs_p(i)) < 999.0) then
          ...
          (i-1) * (nlmax) + 1 + sfields(id%temp)%off
...
! ****************************
    ...
    if (bias_obs_sst/=0.0 .and. mype_filter == 0) &
         write (*, '(a, 5x, a, f12.3)') &
         'FESOM-PDAF', '--- For SST, use global observation bias of ', bias_obs_sst
...
!~        CALL read_syn_obs(file_syntobs_sst, dim_obs, thisobs%obs_f, 0, 1-mype_filter)
!~     END IF
