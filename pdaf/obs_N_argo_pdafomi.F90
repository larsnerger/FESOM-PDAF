...
module obs_n_argo_pdafomi
  ...
  logical :: assim_o_n_argo   !< Whether to assimilate

  ! Further variables specific for the profile observations
  character(len=110) :: path_obs_n_argo  = ''      !< Path to profile observations
  character(len=110) :: file_n_argo

  real    :: rms_obs_n_argo      ! Observation error
  real    :: bias_obs_n_argo     ! Observation bias

  real    :: lradius_n_argo      ! Localization radius
  real    :: sradius_n_argo      ! Support radius for localization function

  real    :: n_argo_exclude_diff ! Limit difference beyond which observations are excluded (0.0 to deactivate)

  real, allocatable :: mean_n_p(:)            ! ensemble mean for observation exclusion
  real, allocatable :: loc_radius_n_argo(:)   ! localization radius array
...
!!
  subroutine init_dim_obs_n_argo(step, dim_obs)
...
! *********************************************
    ...
    lradius_n_argo = 1.0e6 ! 1000 km
    sradius_n_argo = 1.0e6 ! 1000 km
    ...
    thisobs%inno_omit =  n_argo_exclude_diff / rms_obs_n_argo
...
! **********************************
    ...
    file_n_argo = trim(year_string//'-'//mon_string//'-'//day_string//'.'//mype_string//'.nc')
    ...
    if (step < 0) then
        ...
        if (writepe) write (*,'(a,5x,a,1x,i2,a1,i2,a1,i4,1x,f5.2,a,1x,a)') &
            'FESOM-PDAF', 'Postprocessing of DIN ARGO observations at', &
            day_in_month, '.', month, '.', yearnew, timenew/3600.0,&
            'h; read from file:', file_n_argo
       ...
       ! ---
       if (writepe) write (*,'(a,5x,a,1x,i2,a1,i2,a1,i4,1x,f5.2,a,1x,a)') &
            'FESOM-PDAF', 'Assimilate DIN ARGO observations at', &
            day_in_month, '.', month, '.', yearnew, timenew/3600.0,&
            'h; read from file:', file_n_argo
       ...
       if (assim_o_n_argo) thisobs%doassim = 1
    end if
    
    ! complete filename
    filename=trim(path_obs_n_argo)//year_string//'/dist72/'//trim(file_n_argo)
    ...
    if (FileExists) then
       ...
       if (ncstat /= nf90_noerr) then
        print *, 'FESOM-PDAF - obs_n_argo_pdafomi - Error opening NetCDF file'
       end if
       
       ! Get the number of observations dim_obs_p_reps
       ncstat = nf90_inq_dimid(ncid, "INDEX", dimid)
       if (ncstat /= nf90_noerr) then
          print *, "FESOM-PDAF - obs_n_argo_pdafomi - Error getting dimension ID"
       end if
       ncstat = nf90_inquire_dimension(ncid,dimid,len=dim_obs_p_reps)
       if (ncstat /= nf90_noerr) then
          print *, 'FESOM-PDAF - obs_n_argo_pdafomi - Error getting number of observations from NetCDF file'
       end if
    else
       dim_obs_p_reps=0
    endif ! FileExists
    ...
    if (dim_obs_p_reps <= 0) then
      ...
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_n_argo_pdafomi - Error getting id_obs from netCDF'
      ncstat = nf90_get_var(ncid, id_obs, obs_p_reps)
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_n_argo_pdafomi - Error reading obs from netCDF'
      
      ! Reading observation error
      ncstat = nf90_inq_varid(ncid,'ERR', id_err)
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_n_argo_pdafomi - Error getting id_err from netCDF'
      ncstat = nf90_get_var(ncid, id_err, std_obs_p_reps)
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_n_argo_pdafomi - Error reading err from netCDF'
   
      ! Reading nodes
      ncstat = nf90_inq_varid(ncid,'NOD1_P', id_nod1)
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_n_argo_pdafomi - Error getting id_nod1 from netCDF'
      ncstat = nf90_get_var(ncid, id_nod1, nod1_p_reps)
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_n_argo_pdafomi - Error reading nod1 from netCDF'
   
      ncstat = nf90_inq_varid(ncid,'NOD2_P', id_nod2)
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_n_argo_pdafomi - Error getting id_nod2 from netCDF'
      ncstat = nf90_get_var(ncid, id_nod2, nod2_p_reps)
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_n_argo_pdafomi - Error reading nod2 from netCDF'
   
      ncstat = nf90_inq_varid(ncid,'NOD3_P', id_nod3)
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_n_argo_pdafomi - Error getting id_nod3 from netCDF'
      ncstat = nf90_get_var(ncid, id_nod3, nod3_p_reps)
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_n_argo_pdafomi - Error reading nod3 from netCDF'
      
      ncstat = nf90_inq_varid(ncid,'ELEM_P', id_elem)
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_n_argo_pdafomi - Error getting id_elem from netCDF'
      ncstat = nf90_get_var(ncid, id_elem, elem_p_reps)
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_n_argo_pdafomi - Error reading elem from netCDF'
   
      ! Reading layer
      ncstat = nf90_inq_varid(ncid,'NZ1', id_nl)
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_n_argo_pdafomi - Error getting id_layer from netCDF'
      ncstat = nf90_get_var(ncid, id_nl, nl_p_reps)
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_n_argo_pdafomi - Error reading layer from netCDF'
   
      ! Reading coordinates
      ncstat = nf90_inq_varid(ncid,'LON', id_lon)
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_n_argo_pdafomi - Error getting id_lon from netCDF'
      ncstat = nf90_get_var(ncid, id_lon, lon_p_reps)
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_n_argo_pdafomi - Error reading lon from netCDF'
   
      ncstat = nf90_inq_varid(ncid,'LAT', id_lat)
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_n_argo_pdafomi - Error getting id_lat from netCDF'
      ncstat = nf90_get_var(ncid, id_lat, lat_p_reps)
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_n_argo_pdafomi - Error reading lat from netCDF'
      ...
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_n_argo_pdafomi - Error getting id_depth from netCDF'
      ncstat = nf90_get_var(ncid, id_depth, depth_p_reps)
      if (ncstat /= nf90_noerr) print *, 'FESOM-PDAF - obs_n_argo_pdafomi - Error reading depth from netCDF'
      endif ! isPP
    ...
    if (mype_filter == 0) &
            ...
            '--- DIN ARGO observations excluded due to topography: ', cnt_ex_dry
    ...
    call PDAFomi_gather_obs(thisobs, dim_obs_p, obs_p, ivariance_obs_p, ocoord_p, &
                            thisobs%ncoord, lradius_n_argo, dim_obs)
                              
    if (mype_filter == 0) &
        write (*, '(a, 5x, a, i)') 'FESOM-PDAF', &
        '--- Full DIN ARGO observations have been gathered; dim_obs is ', dim_obs
    ...
    if (FileExists) then
       ...
       if (ncstat /= nf90_noerr) then
          print *, 'FESOM-PDAF - obs_n_argo_pdafomi - Error closing NetCDF file'
       end if
    endif
  ...
  end subroutine init_dim_obs_n_argo
...
!!
  subroutine obs_op_n_argo(dim_p, dim_obs, state_p, ostate)
...
!!
  subroutine init_dim_obs_l_n_argo(domain_p, step, dim_obs, dim_obs_l)
    ...
    if (thisobs%doassim == 1) then
       ...
       if (n_sweeps>1) then
          ...
          if (domain_p==1) then
             ...
             if (mype_filter==0) &
                  write (*,'(a,4x,a)') 'FESOM-PDAF', &
                   '--- PHY sweep: set ivar_obs_f for DIN ARGO to 1.0e-12'
          ...
          elseif (domain_p==myDim_nod2D+1) then
             if (mype_filter==0) &
                  write (*,'(a,4x,a)') 'FESOM-PDAF', &
                  '--- BIO sweep: set ivar_obs_f for DIN ARGO to normal'
       ...
       call PDAFomi_init_dim_obs_l(thisobs_l, thisobs, coords_l, &
            locweight, lradius_n_argo, sradius_n_argo, dim_obs_l)
    
     end if ! thisobs%doassim
  end subroutine init_dim_obs_l_n_argo

end module obs_n_argo_pdafomi
