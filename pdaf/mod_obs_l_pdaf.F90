...
!
! !INTERFACE:
  SUBROUTINE cnt_dim_obs_l(wc_coord, lradius, nobs_f_one, ocoord_f_one, nobs_l_one)
...
! !ARGUMENTS:
    REAL, INTENT(in) :: wc_coord(2)          ! Coordinates of current water column
    ...
    REAL, INTENT(in) :: ocoord_f_one(2, nobs_f_one) ! coordinate array for current observations
    INTEGER, INTENT(out) :: nobs_l_one       ! Local dimension of current observation vector
!EOP

! *** Local variables ***
    INTEGER :: i          ! Counter
    REAL :: ocoord(2)     ! Coordinates of observation
...
!//TODO: needs to check whether large radius around pole is necessary
    ! Count local observations
    nobs_l_one = 0
    scanpoints_count: DO i = 1, nobs_f_one

       ! location of observation point
       ocoord(1) = ocoord_f_one(1, i)
       ocoord(2) = ocoord_f_one(2, i)

       ! approximate distances in longitude and latitude
       dist2d(1) = r_earth * ABS(wc_coord(1) - ocoord(1))* COS(wc_coord(2))
       dist2d(2) = r_earth * ABS(wc_coord(2) - ocoord(2))
...
!
! !INTERFACE:
  SUBROUTINE init_obsarrays_l(wc_coord, lradius, nobs_l_one, nobs_f_one, ocoord_f_one, &
       nobs_l_all, id_obs_l_all, dist_l_all, id_obs_l_one, off_obs_l_all, off_obs_f_all)
...
! !ARGUMENTS:
    ...
    INTEGER, INTENT(in) :: nobs_f_one        ! Full dimension of current observation vector
    REAL, INTENT(in) :: ocoord_f_one(2, nobs_f_one) ! coordinate array for current observations
    INTEGER, INTENT(in) :: nobs_l_all        ! Local dimension of observation vector
    INTEGER, INTENT(inout) :: id_obs_l_all(nobs_l_all)   ! index of local obs. in full obs. vector
    REAL, INTENT(inout) :: dist_l_all(nobs_l_all)        ! Distance of obs. from water column
    ...
    IF (nobs_l_one>0) THEN
       scanpoints_count: DO i = 1, nobs_f_one
          ...
          IF (distance2 <= lradius2) THEN
             ...
             id_obs_l_all(off_obs_l_all) = i + off_obs_f_all ! index of local obs. in full obs. vector
             dist_l_all(off_obs_l_all) = SQRT(distance2) ! distance
...
! !INTERFACE:
  SUBROUTINE init_obs_l(nobs_l_all, nobs_l_one, nobs_f_one, id_obs_l_one, &
       obs_f_one, obs_l_all, offset_obs_l_all)
...
!
! The routine has also to return OFFSET_OBS_L_ALL
! incremented by the number of initialized local
! observations. 
!
...
! !ARGUMENTS:
    ...
    INTEGER, INTENT(inout) :: offset_obs_l_all ! input: offset of current observation in obs_l_all and ivar_l_all
                                               ! output: input + nobs_l_one
!EOP
...
! *******************************************
    ...
    ! Increment offset in observation vector for next observation
    offset_obs_l_all = offset_obs_l_all + nobs_l_one
