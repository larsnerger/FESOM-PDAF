...
!********************************************
  ...
  ! * tracers%data(1)%values(:,:)  (nl-1, myDim_nod2D + eDim_nod2D, 1)   ! Temperature
  ! * tracers%data(2)%values(:,:)  (nl-1, myDim_nod2D + eDim_nod2D, 1)   ! Salinity
  ! * uvnode(1,:,:)   (1, nl-1, myDim_nod2D + eDim_nod2D)   ! Velocity u interpolated on nodes
     ...
     do istate = 1, nfields
        ...
        if (sfields(istate)%trnumfesom>0) then
           ...
           do i = 1, myDim_nod2D
              do k = 1, nlmax
                 s = s + 1
                 tracers%data(ifesom)%values(k, i) = state_p(s + sfields(istate)%off)
              enddo
           enddo

           ! initialize external nodes
           call exchange_nod(tracers%data(ifesom)%values(:, :), partit)
