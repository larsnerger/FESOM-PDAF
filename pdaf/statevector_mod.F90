...
! **************************************
    ...
    do i = 1, nfields
       if (sfields(i)%ndims == 1) then
          ...
          write (*, '(a,i2,a)') 'NEMO-PDAF: cannot handle', sfields(i)%ndims, ' number of dimensions.'
       end if
    end do
...
! indeces of 3D and 2D fields
    ...
    DO i=1,nfields
       ...
       ENDIF
    ENDDO
    IF (mype_world==0 .AND. verbose>1) THEN
       DO cnt=1,nfields_3D
...
! *** Write information about the state vector ***
    ...
    IF (mype==0) THEN
       ...
       WRITE (*,'(a,a4,3x,a2,3x,a8,6x,a5,7x,a3,7x,a6,4x,a6,4x,a3)') &
            'NEMO-PDAF','pe','ID', 'variable', 'ndims', 'dim', 'offset', 'update', 'BGC'
    END IF

    IF (mype==0 .OR. (task_id==1 .AND. screen>2)) THEN
       DO i = 1, nfields
          WRITE (*,'(a, i4, i5,3x,a10,2x,i5,3x,i10,3x,i10,4x,l,4x,l)') 'FESOM-PDAF', &
