...
! **********************************
    ...
    ELSE ! (i.e. dim_obs_p > 0)
      ...
      ! indices of observed nodes
        thisobs%id_obs_p(val1,i) = (nlmax) * (nod1_p(i)-1) + nl_p(i) + sfields(id%DIC)%off
        thisobs%id_obs_p(val2,i) = (nlmax) * (nod2_p(i)-1) + nl_p(i) + sfields(id%DIC)%off
        thisobs%id_obs_p(val3,i) = (nlmax) * (nod3_p(i)-1) + nl_p(i) + sfields(id%DIC)%off
